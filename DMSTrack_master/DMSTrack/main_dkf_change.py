...
import sys
sys.path.append('/app/V2X-ATCT/DMSTrack_master/AB3DMOT')
sys.path.append('/app/V2X-ATCT/DMSTrack_master/AB3DMOT/Xinshuo_PyToolbox')
sys.path.append('/app/V2X-ATCT/DMSTrack_master/V2V4Real')
sys.path.append('/app/V2X-ATCT/DMSTrack_master')
sys.path.append('/app/V2X-ATCT/DMSTrack_master/DMSTrack')
...
# from AB3DMOT.scripts.KITTI.evaluate_change import trackingEvaluation, evaluate
...
import os
# os.chdir('/home/maji/Downloads/V2XTargetTracking-main/DMSTrack_master/DMSTrack')  # 切换到指定目录
...
DATA_DIR = '/home/maji/Downloads/V2XTargetTracking-main/V2XGen-main/target_tracking/rq1/results/all/2025-06-13_22:51:18_CIRCLE_1_time'
...
def parse_args():
    ...
    parser.add_argument('--det_name', type=str, default='multi_sensor_differentiable_kalman_filter', help='multi_sensor_differentiable_kalman_filter')
    parser.add_argument('--seq_eval_mode', type=str, default=SEQ_EVAL_MODE, help='which seq to eval: all, 0000, 0001, ...')

    parser.add_argument('--run_evaluation_every_epoch',default=True, action='store_true', help='auto run evaluation every epoch')
    parser.add_argument('--force_gt_as_predicted_track',default=False, action='store_true', help='during training force gt as predicted track before data association and update')
    parser.add_argument('--use_static_default_R', default=False,action='store_true', help='use static default observation noise covariance')
    parser.add_argument('--use_multiple_nets',default=True, action='store_true', help='use multiple observation covariance nets for multiple cars')
    parser.add_argument('--num_frames_backprop', type=int, default=10, help='number of past frames that the loss backpropagates. If -1, loss is calculated for the full (sub) sequence.')
    ...
    parser.add_argument('--load_model_path', type=str, default='./official_models/model_ego_epoch_3.pth', help='path to ego model checkpoint')
    parser.add_argument('--save_dir_prefix', type=str, default='reproducing_official_result', help='prefix of save dir')
    parser.add_argument('--feature', type=str, default=FEATURE, help='model feature, fusion or bev or pos')
    parser.add_argument('--regression_loss_weight', type=float, default=1, help='regression loss weight')
    ...
    parser.add_argument('--use_reversed_cav_id_list',default=False, action='store_true', help='process cav_1 then ego during kf update')
...
def get_global_timestamp_index(seq_name, frame, len_record):
  ...
  # global_timestamp_index = start_global_timestamp_index + 20
  ...
  for sub_seq_idx in sample_sub_seq_indices:
    ...
    for frame in range(min_frame, max_frame + 1):
      ...
      for cav_id in seq_dets_dict.keys():
        dets_frame_dict[cav_id] = get_frame_det(seq_dets_dict[cav_id], frame)#加载txt文件../AB3DMOT/data/v2v4real/detection/multi_sensor_differentiable_kalman_filter_Car_val/ego/0000.txt
                                                                             #../AB3DMOT/data/v2v4real/detection/multi_sensor_differentiable_kalman_filter_Car_val/1/0000.txt
        if 'multi_sensor_differentiable_kalman_filter' in cfg.det_name:
          dets_feature_dict[cav_id] = load_detection_feature(global_timestamp_index, detection_feature_path, cav_id)#加载npy数据，即原始标签
        else:
      ...
      results, affi, loss_dict, matched_detection_id_dict, learnable_R_dict, track_P, det_neg_log_likelihood_loss_dict = tracker.track_multi_sensor_differentiable_kalman_filter(
        dets_frame_dict, frame, seq_name, cav_id_list, dets_feature_dict, gt_boxes, gt_ids, transformation_matrix_dict)#dets_in_gt_order对应  dets_frame_dict； dets_feature对应 dets_feature_dict
      ...
      if save_tracking_results:
        ...
        '''
        保存过去帧与当前帧之间的关联矩阵
        例如，对于 000006.npy 文件，它表示第 5 帧和第 6 帧之间的关联关系
        请注意，关联矩阵中保存的值在实际情况中可能有所不同，因为它是基于原始检测结果和经过自运动补偿后的预测轨迹片段之间计算得出的，而非基于实际输出的两组轨迹片段。
        
        '''
        save_affi_file = os.path.join(affinity_dir, '%06d.npy' % frame)
        # print(save_affi_file)
...
'''
track_and_evaluate 函数的主要目的是使用训练好的模型在评估集上进行目标跟踪和评估，并且不进行损失的反向传播和优化器的参数更新。
它会创建评估结果的保存目录，调用 train_one_epoch 函数进行跟踪和损失计算，保存模型的检查点，最后调用评估函数计算评估指标。
'''
def track_and_evaluate(save_dir_prefix, cav_id_list, observation_covariance_net_dict, evaluation_ground_truth_data_all, evaluation_config_dict, evaluation_split, cfg, seq_eval_mode, epoch_idx, cat, ID_start, hw, log, dtype, device, differentiable_kalman_filter_config, det_id2str, force_gt_as_predicted_track, use_static_default_R, use_multiple_nets, show_vis, save_vis, mail, threshold_3D_IOU, loss_types, writer, regression_loss_weight, association_loss_weight, det_neg_log_likelihood_loss_weight):
  ...
  Similar to the code calling train_one_epoch, but do not call loss.backprop() and optimizer.step()
    ...
    save_dir_prefix：保存评估结果的目录前缀。
    cav_id_list：协同感知车辆 ID 列表。
    observation_covariance_net_dict：观测协方差网络字典，键为 cav_id，值为对应的模型。
    evaluation_ground_truth_data_all：评估集的所有真实标签数据。
    evaluation_config_dict：评估配置字典，包含结果哈希值、检测根目录等信息。
    evaluation_split：评估集划分（如 'val'）。
    cfg：配置对象，包含各种配置参数。
    seq_eval_mode：序列评估模式（如 'all'、'0001' 等）。
    epoch_idx：当前的训练轮数。
    cat：目标类别（如 'Car'）。
    ID_start：目标 ID 的起始值。
    hw：硬件信息字典。
    ...
    differentiable_kalman_filter_config：可微分卡尔曼滤波的配置字典。
    det_id2str：目标类别 ID 到名称的映射字典。
    force_gt_as_predicted_track：是否强制使用真实标签作为预测的跟踪结果。
    use_static_default_R：是否使用静态的默认观测协方差矩阵。
    use_multiple_nets：是否使用多个网络。
    show_vis：是否显示可视化结果。
    save_vis：是否保存可视化结果。
    mail：邮件相关设置。
    threshold_3D_IOU：3D 交并比阈值。
    loss_types：损失类型列表。
    writer：用于记录训练信息的写入器。
    regression_loss_weight：回归损失的权重。
    association_loss_weight：关联损失的权重。
    det_neg_log_likelihood_loss_weight：检测负对数似然损失的权重。

  '''
  '''

    torch.no_grad()：上下文管理器，用于禁用梯度计算，减少内存消耗并提高计算速度。
    model.eval()：将模型设置为评估模式，关闭一些在训练时使用的特殊层（如 Dropout 和 BatchNorm）。
  ...
  with torch.no_grad():
    ...
    根据评估配置和当前轮数生成评估结果保存文件夹的名称。
    使用 os.path.join 函数拼接保存目录的路径。
    mkdir_if_missing 函数用于创建目录，如果目录不存在的话。
    ...
    # evaluation_save_folder = 'evaluation_' + evaluation_config_dict['result_sha'] + '_%s' % seq_eval_mode + '_H%d' % cfg.num_hypo + '_epoch_%d' % epoch_idx + '_feature_%s' % FEATURE + '_time_%s' % TIME
    ...
    # 为每个假设（cfg.num_hypo）创建一个子目录，用于保存对应的评估结果。
    eval_dir_dict = dict()
    for index in range(cfg.num_hypo):
    ...
    save_tracking_results：是否保存跟踪结果。
    调用 train_one_epoch 函数进行跟踪和损失计算，将 is_training 参数设置为 False，表示不进行训练。
    传入各种配置参数和数据，得到平均评估损失和损失统计字典。
    ...
    # 如果 run_evaluation_every_epoch 为 True，则调用 evaluate 函数对当前评估集的跟踪结果进行评估，得到评估指标字典。
  ...
  sys.stdout.flush()
  return evaluation_metrics_dict, evaluation_loss_stats_avg, evaluation_loss_stats_avg_dict,evaluation_save_dir
...
def get_config(cfg, cat, training_split, evaluation_split, seq_eval_mode, use_reversed_cav_id_list):
  ...
  # train_det_root = '/home/maji/Downloads/DMSTrack_master/V2V4Real/official_models/train_no_fusion_keep_all/npy/ab3dmot_detection'

  val_result_sha = '%s_%s_%s' % (cfg.det_name, cat, 'val')
  # val_det_root = os.path.join('../AB3DMOT/data', cfg.dataset, 'detection', val_result_sha)
  # val_det_root = '/home/maji/Downloads/DMSTrack_master/V2V4Real/official_models/no_fusion_keep_all/npy/ab3dmot_detection'
  # val_det_root = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/no_fusion_keep_all/npy/ab3dmot_detection'
  # val_det_root = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/no_fusion_keep_all/npy/ab3dmot_detection'
  ...
  if 'multi_sensor_differentiable_kalman_filter' in cfg.det_name:
    ...
    # val_detection_feature_path = '../V2V4Real/official_models/no_fusion_keep_all/npy/' #修改为自己的路径
    # val_detection_feature_path = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/no_fusion_keep_all/npy/'
    # val_detection_feature_path = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/ori_data/npy/' #原始数据
    val_detection_feature_path = os.path.join(DATA_DIR,'npy')
  ...
  val_det_root = os.path.join(val_detection_feature_path,'ab3dmot_detection')
...
def main_per_cat_multi_sensor_differentiable_kalman_filter(cfg, cat, log, ID_start, dtype, device, run_evaluation_every_epoch, seq_eval_mode, force_gt_as_predicted_track, use_static_default_R, use_multiple_nets, show_vis, save_vis, num_frames_backprop, num_frames_per_sub_seq, num_epochs, load_model_path, save_dir_prefix, training_split, evaluation_split, feature, regression_loss_weight, association_loss_weight, det_neg_log_likelihood_loss_weight, clip_grad_norm, use_reversed_cav_id_list):
  ...
  # print(evaluation_ground_truth_data_all)
  # return

 

  tensorboard_save_folder =  'training_' + training_config_dict['result_sha'] + '_%s' % seq_eval_mode + '_H%d' % cfg.num_hypo + '_epoch_%d' % 0
  # tensorboard_save_folder =  'training_' + training_config_dict['result_sha'] + '_%s' % seq_eval_mode + '_H%d' % cfg.num_hypo + '_epoch_%d' % 0 + '_feature_%s' % FEATURE + '_time_%s' % TIME
  ...
  # tensorboard_save_folder =  'training_' + training_config_dict['result_sha'] + '_%s' % seq_eval_mode + '_H%d' % cfg.num_hypo + '_epoch_%d' % 0
  # tensorboard_save_dir = os.path.join(DATA_DIR, tensorboard_save_folder, 'tensorboard'); mkdir_if_missing(tensorboard_save_dir)
  ...
  if run_init_evaluation:
    ...
    # let epoch_idx = 0
    evaluation_metrics_dict, evaluation_loss_stats_avg, evaluation_loss_stats_avg_dict,evaluation_save_dir = track_and_evaluate(save_dir_prefix, cav_id_list, observation_covariance_net_dict, evaluation_ground_truth_data_all, evaluation_config_dict, evaluation_split, cfg, seq_eval_mode, 0, cat, ID_start, hw, log, dtype, device, differentiable_kalman_filter_config, det_id2str, force_gt_as_predicted_track, use_static_default_R, use_multiple_nets, show_vis, save_vis, mail, threshold_3D_IOU, loss_types, writer, regression_loss_weight, association_loss_weight, det_neg_log_likelihood_loss_weight)
  ...
  for epoch_idx in range(1, num_epochs+1):
    ...
    # training_save_dir = os.path.join(DATA_DIR, training_save_folder); mkdir_if_missing(training_save_dir)
  ...
  return ID_start,evaluation_save_dir
...
def main(args):
  ...
  # config_path = '/home/maji/Downloads/DMSTrack_master/DMSTrack/configs/v2v4real.yml' 
  ...
  # print configs
  # 生成日志文件
  time_str = get_timestring()#获取当前时间字符串
  ...
  evaluation_save_dirs = []

  for cat in cfg.cat_list:
    ID_start,evaluation_save_dir = main_per_cat_multi_sensor_differentiable_kalman_filter(cfg, cat, log, ID_start, dtype, device, 
    ...
    evaluation_save_dirs.append(evaluation_save_dir)
  ...
  print_log('\nDone!', log=log)
  log.close()
  return evaluation_save_dirs

if __name__ == '__main__':
  
  args = parse_args()
  # print(args.det_name)
  evaluation_save_dirs = main(args)
  print(evaluation_save_dirs)
