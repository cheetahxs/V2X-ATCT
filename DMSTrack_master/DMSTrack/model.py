...
class ObservationCovarianceNet(torch.nn.Module):
  ...
  def forward(self, x, frame, transformation_matrix, dets_in_gt_order):
    ...
    '''
    # print(x.shape) #torch.Size([6, 320, 20, 20])
    # print(dets_in_gt_order.shape) #torch.Size([5, 7])
    N, C, W, H = x.shape
    if N == 0:
  ...
  def forward_fusion(self, x, frame, transformation_matrix, dets_in_gt_order):
    ...
    # positional encoding
    # 包含了检测框的位置、方向和距离等信息的位置嵌入。N 是检测框的数量
  ...
  def get_positional_embedding(self, transformation_matrix, dets_in_gt_order, hidden_dim=256):
    ...
    Input
      ...
      transformation_matrix：形状为 [4, 4] 的张量，表示从协作自动驾驶车辆（CAV）传感器坐标系到自车坐标系的变换矩阵。
      dets_in_gt_order：形状为 [N, 7] 的张量，其中 N 是检测目标的数量，每个检测目标用 7 个值表示，分别是 [x, y, z, theta, l, w, h]，表示目标在自车坐标系中的位置、朝向和尺寸。
      hidden_dim：可选参数，默认值为 256，表示位置嵌入的维度。
    ...
    Output:
      ...
      positional_embedding：形状为 [N, (7 + 1 + 5 + 5), K=positional_dim=256] 的张量，包含了目标的位置、距离等信息的编码。
      distance_2d_det_to_ego：形状为 [N] 的张量，表示每个检测目标到自车的二维距离。
      distance_2d_det_to_sensor：形状为 [N] 的张量，表示每个检测目标到传感器的二维距离。
      distance_2d_sensor_to_ego：标量，表示传感器到自车的二维距离。

    get_positional_embedding 是一个类方法（从 self 参数可以推断），其主要功能是根据给定的变换矩阵和检测目标的信息，生成位置嵌入（positional embedding）。
    位置嵌入是在深度学习中常用的一种技术，用于将位置信息编码成高维向量，以便模型能够更好地理解和利用位置信息。
...
class DMSTrack(AB3DMOT):
  ...
  def track_multi_sensor_differentiable_kalman_filter(self, dets_all_dict, frame, seq_name, cav_id_list, dets_feature_dict, gt_boxes, gt_ids, transformation_matrix_dict):
    ...
    dets_all_dict：字典
        dets_all_dict[cav_id] 是一个字典，其中 cav_id 来自 cav_id_list。
            dets：一个以 [[h, w, l, x, y, z, theta], ...] 格式存储的检测结果的 NumPy 数组。
            info：一个包含每个检测结果其他信息的数组。
        frame：字符串，帧编号，用于查询自车的位姿。
        cav_id_list：按照 cav_id_list 的顺序将跟踪结果与各车辆的检测结果进行匹配。
        dets_feature_dict：每个目标的目标检测特征。
        gt_boxes：真实框 [N, 7]，格式为 [x, y, z, theta, l, w, h]。
        gt_ids：真实目标的 ID：[N]，其中 N 为当前帧中的目标数量。
    要求：即使检测结果为空，该方法也必须针对每一帧调用一次。
    返回值：返回一个类似的数组，其最后一列是目标 ID。
    ...
    for cav_id in dets_all_dict.keys():
      ...
      if self.use_multiple_nets:
        ...
        observation_covariance_dict[cav_id] = self.observation_covariance_net_dict['ego'](
          dets_feature, frame,  transformation_matrix, dets_in_gt_order)#1-4有问题
