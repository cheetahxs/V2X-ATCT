...
sys.path.append("/app/DMSTrack_master")
sys.path.append("/app/DMSTrack_master/AB3DMOT")
sys.path.append('/app/DMSTrack_master/AB3DMOT/Xinshuo_PyToolbox')
sys.path.append('/app/DMSTrack_master/DMSTrack')
import shutil
import os
import random
import os
...
import DMSTrack_master.DMSTrack.main_dkf_change as dkf
import fusion_followcar_overtake_action as overtake
import follow_car_action as followcar
import turning_action as turning

import DMSTrack_master.V2V4Real.opencood.tools.inference_new as inference

import yaml
import DMSTrack_master.V2V4Real.opencood.tools.inference as inference_all
import json
import secrets
import DMSTrack_master.AB3DMOT.main_change as cobevtmain
import DMSTrack_master.AB3DMOT.scripts.KITTI.evaluate as evaluate
import rq1
# import os
...
def gen_data(
    save_path='/app/V2X-ATCT/target_tracking/rq2',
    scene = 1,
    sharding = 30,
    gen_seed_num = 5,
    carnum=3,
...
def gen_all_data(
    save_path_root='V2X-ATCT/target_tracking/rq2',
    gen_time = 9,
    sharding = 31,
    gen_seed_num = 5,
    speed=60,
...
def gen_single_data(
    ...
    if action == 'overtake':
            ...
            # print('overtake')
            save_dir1,time1,index_list = overtake.main(dx_val=dx_val,i=scene_num,save_path=save_path_guide,gen_data_for_sharding=False,sharding=sharding,car_num=carnum)
            save_dir2,time2,index_list = overtake.main(dx_val=dx_val,i=scene_num,save_path=save_path_baseline,gen_data_for_base_line=False,sharding=sharding,index_list_for_baseline=index_list,car_num=carnum)
            if gen_seed_num >1:
                for index in range(gen_seed_num-1):
                    overtake.main(dx_val=dx_val,i=scene_num,save_path=save_path_guide,gen_data_for_base_line=False,sharding=sharding,index_list_for_baseline=index_list,car_num=carnum)


    elif action == 'followcar':
        
            # print('followcar')
            save_dir1,time1,index_list = followcar.main(dx_val=dx_val,i=scene_num,save_path=save_path_guide,gen_data_for_sharding=False,sharding=sharding,car_num=carnum)
            save_dir2,time2,index_list = followcar.main(dx_val=dx_val,i=scene_num,save_path=save_path_baseline,gen_data_for_base_line=False,sharding=sharding,index_list_for_baseline=index_list,car_num=carnum)
            if gen_seed_num >1:
                for index in range(gen_seed_num-1):
                    followcar.main(dx_val=dx_val,i=scene_num,save_path=save_path_guide,gen_data_for_base_line=False,sharding=sharding,index_list_for_baseline=index_list,car_num=carnum)


    elif action == 'turning':
        
            save_dir1,time1,index_list,start_ratio = turning.main(dx_val=dx_val,i=scene_num,save_path=save_path_guide,gen_data_for_sharding=False,sharding=sharding,car_num=carnum)
            # print(index_list)
            save_dir2,time2,index_list,start_ratio = turning.main(dx_val=dx_val,i=scene_num,save_path=save_path_baseline,gen_data_for_base_line=False,sharding=sharding,index_list_for_baseline=index_list,start_ratio=start_ratio,gen_data_for_sharding=False,car_num=carnum)
            if gen_seed_num >1:
                for index in range(gen_seed_num-1):
                    turning.main(dx_val=dx_val,i=scene_num,save_path=save_path_guide,gen_data_for_base_line=False,sharding=sharding,index_list_for_baseline=index_list,start_ratio=start_ratio,gen_data_for_sharding=False,car_num=carnum)
    else :
        print('error')
        raise TypeError
...
def gen_data_and_test(
        ...
        config_dir = '/app/V2X-ATCT/target_tracking/late_fusion',
        fusion_method = 'no_fusion_keep_all',
        only_test = False,
        # data_dir = '',
        seed_num = 5,
        save_path_root='/app/V2X-ATCT/target_tracking/rq2/',
        scene_num = 9,
        sharding = 31,
        select_seed_num=1,
        insert_time=1,
    ...
    print(script_dir)
    config_dir = os.path.join(script_dir,'V2X-ATCT/target_tracking/late_fusion')
    save_path_root = os.path.join(script_dir,'V2X-ATCT/target_tracking/rq2')
...
def args_parser():
    ...
    parser.add_argument('--save_path', type=str, required=False, default='/app/V2X-ATCT/target_tracking/rq2'
                        )
    parser.add_argument('--seed_num', type=int, required=False, default=1
                        )
    parser.add_argument('--system', type=str, required=False,default='all'
                        )
    parser.add_argument('--select_seed_num', type=int, required=False, default=1
                        )
    parser.add_argument('--driving_behaviour', type=str, required=False,default='all'
                        )
    parser.add_argument('--insert_time', type=int, required=False,default=1
                        )
...
if __name__ == '__main__':
    ...
    gen_data_and_test(save_path_root=save_path,seed_num=seed_num,carnum=carnum,speed=speed,only_test=False)
