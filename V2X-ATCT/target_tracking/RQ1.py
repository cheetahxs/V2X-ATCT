...
sys.path.append('/app/DMSTrack_master/V2V4Real')
sys.path.append('/app/DMSTrack_master')
sys.path.append('/app/DMSTrack_master/DMSTrack')
sys.path.append("/app")

# sys.path.append("/home/maji/Downloads/docker/V2X-ATCT")
# sys.path.append("/home/maji/Downloads/docker/V2X-ATCT/DMSTrack_master")
# sys.path.append("/home/maji/Downloads/docker/V2X-ATCT/DMSTrack_master/AB3DMOT")
# sys.path.append('/home/maji/Downloads/docker/V2X-ATCT/DMSTrack_master/AB3DMOT/Xinshuo_PyToolbox')
# sys.path.append('/home/maji/Downloads/docker/V2X-ATCT/DMSTrack_master/DMSTrack')
...
# # 检查 DMSTrack 是否在 /app 下
# print("🔍 Does /app/DMSTrack exist?", os.path.exists("/app/DMSTrack"))
...
import target_tracking.RQ2 as rq2
...
def copy_folder(src, dst, overwrite=False):
    ...
    if os.path.exists(dst):
        if not os.path.isdir(dst):
            print(f"❌ Error: Destination is not a directory: {dst}")
            return False
            
        if not overwrite:
            print(f"❌ Error: Destination directory already exists (use overwrite=True to replace): {dst}")
            return False
        else:
            print(f"🗑️  Removing existing directory: {dst}")
            try:
                shutil.rmtree(dst)
            except Exception as e:
...
def gen_data_and_npy(
    ...
    if only_test == False:
        data_dir = gen_all_data(time=time,SAVE_PATH_ROOT='./V2X-ATCT/target_tracking/rq1/',v2x_dataset_path=v2x_dataset_path,carnum=carnum,speed=speed)
...
def main(save_path_dir = 'rq1',
    ...
    # print("脚本文件所在目录:", os.path.dirname(__file__))
    # print("使用 pathlib:",Path(__file__).parent.absolute())

    now = datetime.now()
    formatted = now.strftime("%Y-%m-%d_%H:%M:%S")
    SAVE_PATH = os.path.join(save_path_dir,f'RQ1_EXP_{formatted}')

    v2x_dataset = os.path.join('/app/V2X-ATCT/data/v2x_dataset')
    ...
    for i in range(0,insert_time):
        ...
        os.system(f'python dataset_init.py -d {select_seed_path}')
        v2x_dataset = os.path.join(select_seed_path,'v2x_dataset')

        
    for seed in select_seeds:
        trans_data_to_npy(data_dir=seed,config_dir='/app/V2X-ATCT/target_tracking/late_fusion',fusion_method= 'no_fusion_keep_all')
...
def args_parser():
    ...
    parser.add_argument('--save_path_dir', type=str, required=False,default='/app/V2X-ATCT/target_tracking/rq1'
                        )
    parser.add_argument('--gen_seed_num', type=int, required=False,default=1
                        )
    parser.add_argument('--insert_time', type=int, required=False,default=3
                        )
...
if __name__ == '__main__':
    ...
    v2x_dataset_path = '/app/V2X-ATCT/data'
