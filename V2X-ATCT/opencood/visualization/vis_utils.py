...
def visualize_single_dataloader(dataloader, order, num,color_mode='constant'):
    """
...
def visualize_sequence_dataloader(dataloader, order, color_mode='constant'):
    ...
    # ctr = vis.get_view_control()
    # ctr.set_zoom(0.1)
    # ctr.set_lookat(np.array([0, 0, 55.0])) # 值越小，视图放大越多
    ...
    while True:
        for i_batch, sample_batched in enumerate(dataloader):
            # if i_batch < begin_index:
            #     continue
            # if i_batch > end_index:
            #     continue
            print(i_batch)
            pcd, aabbs = \
                visualize_single_sample_dataloader(sample_batched['ego'],
