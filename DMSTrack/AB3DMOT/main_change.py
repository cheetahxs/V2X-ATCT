...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='v2v4real', help='KITTI, nuScenes')
    parser.add_argument('--split', type=str, default='', help='train, val, test')
    parser.add_argument('--det_name', type=str, default='cobevt', help='pointrcnn')
...
def main_per_cat(cfg, cat, log, ID_start):
	...
	det_root = os.path.join('./data', cfg.dataset, 'detection', result_sha)#输入路径
	...
	save_dir = os.path.join(cfg.save_root, result_sha + '_H%d' % cfg.num_hypo); mkdir_if_missing(save_dir)#保存路径
	...
	for index in range(cfg.num_hypo):
		eval_dir_dict[index] = os.path.join(save_dir, 'data_%d' % index); mkdir_if_missing(eval_dir_dict[index]) #保存路径		
	print('eval_dir_dict[0]: ', eval_dir_dict[0])
	# ./results/v2v4real/cobevt_Car_val_H1/data_0

	# loop every sequence
	# seq_eval = [EVAL_SEQ]#修改，只计算一个序列
