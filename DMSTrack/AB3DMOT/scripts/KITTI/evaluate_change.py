...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="../AB3DMOT/scripts/KITTI", max_truncation = 0, min_height = 25, max_occlusion = 2, \
        ...
        seq_eval_mode='all', v2v4real_split='val',sharding = 25):
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        if not loading_groundtruth:
            ...
            self.dcareas     = []
            self.groundtruth = []
            # print(f'seq_data2: {len(seq_data)}')
    ...
    # def compute3rdPartyMetrics(self, threshold=3):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            print(f"seq_gt:{len(seq_gt)}")

            # for f in range(len(seq_gt)):   
            for f in range(30):        # go through each frame
                g = seq_gt[f]
                dc = seq_dc[f]
    ...
    # def compute3rdPartyMetrics(self, threshold=3):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):        # go through each frame
                ...
                # mapping for tracker ids and ground truth ids
                for row,col in association_matrix: #行列匹配
                    # apply gating on box overlap
                    c = cost_matrix[row][col]
                    if c < max_cost:
                        g[row].tracker   = t[col].track_id
                    ...
                    # print('g[row]:',g[row].distance)
                    # print('g[row]:',g[row].valid)
                    # print('g[row]:',g[row].tracker)
                    print('g[row]:',g[row].x,g[row].y,g[row].z)
                    # print(row)
...
def evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres, evaluate_v2v4real=False, seq_eval_mode='all', v2v4real_split='val',sharding=25):
    """
    ...
    for c in class_list:
        e = trackingEvaluation(t_sha=result_sha, mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou,num_hypo=num_hypo,thres=thres, evaluate_v2v4real=evaluate_v2v4real, seq_eval_mode=seq_eval_mode, v2v4real_split=v2v4real_split,sharding=sharding)
        ...
        # e.compute3rdPartyMetrics_rq2(best_threshold)
        e.saveToStats(dump) 
