...
# import os
# os.chdir('/home/maji/Downloads/V2XTargetTracking-main/DMSTrack_master/V2V4Real')
...
#超车，场景2
# MODEL_DIR = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/2025-05-15_13:01:48/' #转换数据路径
# MODEL_DIR = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/2025-05-13_18:58:15/'

# #停车，场景4
# MODEL_DIR = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/2025-05-08_11:33:47/'

# #转向，场景4
# MODEL_DIR = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/2025-04-10_11:16:21/'

#原始场景
# MODEL_DIR = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/scene1/'
# MODEL_DIR = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test/scene2/'
# SCENE = 2
...
def test_parser(model_dir='',fusion_method='no_fusion_keep_all'):
...
def main(
    ...
    # MODEL_DIR = model_dir
    # FUSION_METHOD = fusion_methon
    
    # print(FUSION_METHOD)

    opt = test_parser(fusion_method=fusion_methon,model_dir=config_path)
    ...
    # opt.model_dir = '/home/maji/Downloads/DMSTrack_master/V2V4Real/data/test' #配置文件放在固定路径
    # opt.model_dir = config_path 

    hypes = yaml_utils.load_yaml(None, opt)
    source_pth = os.path.join(opt.model_dir,'net_epoch60.pth')
    destination_pth = os.path.join(data_dir,'net_epoch60.pth')
    shutil.copy(source_pth,destination_pth)



    opt.model_dir = data_dir
    hypes['validate_dir'] = data_dir#重新修改数据路径
    ...
    #                     0.7: {'tp': [], 'fp': [], 'gt': 0}}
    result_stat = {0.5: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0},
                   0.7: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0}}
    result_stat_short = {0.5: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0},
                         0.7: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0}}
    result_stat_middle = {0.5: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0},
                          0.7: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0}}
    result_stat_long = {0.5: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0},
                        0.7: {'tp': [], 'fp': [], 'gt': 0,'sumIOU':0,'numIOUs':0}}
    ...
    index_list = [147, 261, 405, 603, 783, 1093, 1397, 1618, 1993]#设置编号
    if scene == 1:
       first_index = 0
       end_index = index_list[scene-1]
    else :
       first_index = index_list[scene-2]
       end_index = index_list[scene-1]

    for index, batch_data in enumerate(data_loader):
        print(index)
        i = first_index+index
        # print(batch_data['ego'])
        # return
        ...
        with torch.no_grad():
            ...
            if opt.save_npy:
                ...
                if opt.fusion_method == 'no_fusion_keep_all' or opt.fusion_method=='intermediate':
                  for cav_id in pred_box_dict.keys():
    ...
    # MY_CODE
    if opt.save_npy:
      transform_and_save_detection_to_ab3dmot_format(opencood_dataset.len_record, npy_save_path, cav_id_set,scene=scene)
      transform_and_save_tracking_label_to_ab3dmot_format(opencood_dataset.len_record, npy_save_path,scene=scene)

    if opt.show_sequence:
        vis.destroy_window()

def transform_and_save_detection_to_ab3dmot_format(len_record, npy_save_path, cav_id_set,scene):
  ...
  for cav_id in cav_id_set:
    ...
    index_list = [147, 261, 405, 603, 783, 1093, 1397, 1618, 1993]
    if scene == 1:
       first_index = 0
       end_index = index_list[scene-1]-1
    else :
       first_index = index_list[scene-2]
       end_index = index_list[scene-1]-1
    ...
    for scenario_index in range(1):
      start_global_timestamp_index = first_index
      end_global_timestamp_index = end_index
      ...
      ab3dmot_detection_save_file = os.path.join(ab3dmot_detection_save_path, '%04d.txt' % (scene-1))
...
def transform_and_save_tracking_label_to_ab3dmot_format(len_record, npy_save_path,scene):
  # opencood_dataset.len_record
  print('len_record: ', len_record)
  # [147, 261, 405, 603, 783, 1093, 1397, 1618, 1993]
  ...
  for scenario_index in range(1):
    ...
    ab3dmot_tracking_label_save_file = os.path.join(ab3dmot_tracking_label_save_path, '%04d.txt' % (scene-1))
