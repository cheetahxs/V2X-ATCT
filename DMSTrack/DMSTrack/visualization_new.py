import sys
sys.path.append('/home/maji/Downloads/DMSTrack_master/AB3DMOT')
sys.path.append('/home/maji/Downloads/DMSTrack_master/AB3DMOT/Xinshuo_PyToolbox')
sys.path.append('/home/maji/Downloads/DMSTrack_master/V2V4Real')
sys.path.append('/home/maji/Downloads/DMSTrack_master')
...
from V2V4Real.opencood.visualization.vis_utils import color_encoding, bbx2linset, LineMesh,lineset_assign
...
def custom_draw_geometry(pcd_list, boxes_list, seq_name):
    ...
    new_camera_params_file = os.path.join('./visualization_parameters', camera_file)
...
def visualize_single_sample_output_gt(
  ...
  frame=''):
    ...
    if save_vis:
        ...
        # }
        # with open(pcd_and_boxes_save_path, "wb") as f:
        #   pickle.dump(scene, f)
        save_pcd_and_boxes(save_vis_dir=save_vis_dir,seq_name=seq_name,frame=frame,pcd_list=pcd_list,boxes_list=boxes_list)
...
def save_pcd_and_boxes(save_vis_dir='/home/maji/Downloads/DMSTrack_master/DMSTrack/results/v2v4real/reproducing_official_result/evaluation_multi_sensor_differentiable_kalman_filter_Car_val_0000_H1_epoch_0/visualization',
                       seq_name=0,frame=20,pcd_list=[],boxes_list=[]):
    save_path_base = os.path.join(save_vis_dir,f'seq_{seq_name}/frame_{frame:04d}')
    # print(save_path_base)
    # return
    if not os.path.exists(save_path_base):
      os.makedirs(save_path_base)
    for index, pcd in enumerate(pcd_list):
      save_pcd_path = os.path.join(save_path_base,f'{index:04d}.pcd')  
      # 持久化保存点云数据到文件
      try:
        # 使用 open3d.io.write_point_cloud 函数保存点云
        o3d.io.write_point_cloud(save_pcd_path, pcd)
        
      except Exception as e:
        print(f"保存点云数据时出错: {e}")      

    for number,boxes in enumerate(boxes_list):
        for index, ele in enumerate(boxes):
          save_box_path = os.path.join(save_path_base,f'{number}_{index}.ply')
          # 持久化保存 LineSet 对象到文件
          try:
              # 使用 o3d.io.write_line_set 函数保存 LineSet 对象
              # o3d.io.write_line_set(save_box_path, ele)
              o3d.io.write_triangle_mesh(save_box_path,ele)
              
          except Exception as e:
              print(f"保存 LineSet 数据时出错: {e}")
       
def load_pcd_and_boxes(save_vis_dir='/home/maji/Downloads/DMSTrack_master/DMSTrack/results/v2v4real/reproducing_official_result/evaluation_multi_sensor_differentiable_kalman_filter_Car_val_0000_H1_epoch_0/visualization',
                       seq_name='0000',
                       frame=0):
  pcd_list = []
  boxes_list = []
  load_path_base = os.path.join(save_vis_dir,f'seq_{seq_name}/frame_{frame:04d}')
  # 遍历文件夹
  for root, dirs, files in os.walk(load_path_base):
      for file in files:
  ...
  for root, dirs, files in os.walk(load_path_base):
      for file in files:
          if file.endswith('.ply'):
              file_path = os.path.join(root, file)
              # print(file_path)
              ele = mesh = o3d.io.read_triangle_mesh(file_path)
              boxes_list.append(ele)

  return pcd_list,boxes_list
  # loaded_pcd = o3d.io.read_point_cloud(save_path)

def vis_data_sequence():
  ...
  for frame in range(0,147):
    print(frame)
    pcd_list,boxes_list=load_pcd_and_boxes(save_vis_dir='/home/maji/Downloads/DMSTrack_master/DMSTrack/results/v2v4real/reproducing_official_result/evaluation_multi_sensor_differentiable_kalman_filter_Car_val_0000_H1_epoch_0/visualization',
                       seq_name='0000',
                       frame=frame)
    ...
    if frame == 0:
      ...
      for i in range(len(vis_aabbs)):
        index = i if i < len(boxes_list) else -1
        vis_aabbs[i] = lineset_assign(vis_aabbs[i], boxes_list[index])
...
def visualize(show_vis, save_vis, save_vis_dir, ab3dmot_tracking_results, gt_boxes, gt_ids, 
  ...
  #持久化结果
  pcd_and_boxes_file_name = 'seq_%s_frame_%04d.pkl' % (seq_name, frame)
  pcd_and_boxes_save_path = os.path.join(save_vis_dir, save_file_name)

  # print(save_file_name)

  visualize_single_sample_output_gt(seq_name, trk_boxes, gt_boxes, pcd, show_vis=show_vis, save_vis=save_vis, save_path=save_path, det_boxes_dict=det_boxes_dict, gt_ids=gt_ids, trk_ids=trk_ids, matched_detection_id_dict=matched_detection_id_dict, learnable_R_dict=learnable_R_dict, track_P=track_P,save_vis_dir=save_vis_dir,frame=frame)
